...
namespace Poggus.Player
{
	public class RightSwordLinkState : ILinkState
	{
        ...
        public RightSwordLinkState(ILink Link, ISprite sprite)
		{
            ...
            link.ProjectileFactory.NewStab(LocationHelpers.GetLocationCenteredSpawnRight(link.DestRect, ProjectileConstants.horizSwordBeamSize), Direction.right);

            //Fire a sword beam if link is at full health
            if (link.FullHealth())
            {
                link.ProjectileFactory.NewSwordBeam(LocationHelpers.GetLocationCenteredSpawnRight(link.DestRect, ProjectileConstants.horizSwordBeamSize), Direction.right);
